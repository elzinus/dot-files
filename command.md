...
- ``sudo gdebi <packagename>`` -- use command to install packages deb from /tmp
- Latex Pandox ``\pagenumbering{gobble}`` -- latex code for disableing pagenumbers. Put in your markdown document as first line before conversion with pandoc to pdf
- ``wajig large`` -- om alle geinstalleerde packages op grote te tonen
